...
function curriculumPage(){
  ...
  app.innerHTML=`<div class="section-head"><div><h2>CP · TP · ATP · Program Semester · Program Tahunan</h2><p class="muted">Rantai perangkat dibuat saling terhubung: CP → TP → ATP → PROMES → PROTA. Paket lengkap juga menyiapkan Modul Ajar, LKPD, Bahan Ajar, asesmen, rubrik, KKTP, remedial, pengayaan, refleksi, glosarium, dan halaman pengesahan.</p></div><div><button class="btn" onclick="window.print()">🖨 Print</button> <button class="btn primary-dark" onclick="generateAllCurriculum()">⚡ Generate 5 Dokumen</button><button class="btn" onclick="generateCompletePackage()">✨ Paket Lengkap</button></div></div>
  ${contextBar()}
...
async function generateCompletePackage(){
  const mode=document.getElementById('curriculumMode');
  if(mode)mode.textContent='Menyiapkan paket lengkap…';
  showToast('Menyiapkan CP → TP → ATP → PROMES → PROTA → Modul Ajar + Lampiran…');
  ...
  try{
    for(const type of types){
      ...
      const data=await r.json(); if(!r.ok)throw new Error(data.error||`Gagal membuat ${type}`);
    ...
    const modulePayload={
      level:state.level,grade:Number(state.grade),section:state.section,subject:state.subject,
      chapter:document.getElementById('curriculumChapter')?.value||state.subject,
      semester:state.semester,academicYear:state.academicYear,
      allocationMinutes:Number(document.getElementById('curJpMeeting')?.value||2)*Number(document.getElementById('curMeetingMinutes')?.value||35),
      model:'AUTO',context:'Gunakan CP, TP, ATP, PROMES, dan PROTA yang baru dibuat sebagai rantai acuan.'
    };
    modulePayload.linkedCP=chain.CP.content; modulePayload.linkedTP=chain.TP.content; modulePayload.linkedATP=chain.ATP.content; modulePayload.linkedProsem=chain['Program Semester'].content; modulePayload.linkedProta=chain['Program Tahunan'].content;
    const mr=await fetch('/api/generate-module-sections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(modulePayload)});
    const md=await mr.json(); if(!mr.ok)throw new Error(md.error||'Gagal membuat Modul Ajar');
    moduleSections={...moduleSections,...(md.sections||{}),chapter:modulePayload.chapter};
    localStorage.setItem('module_sections',JSON.stringify(moduleSections));
    allSources.push(...(md.sources||[]));
    const result=document.getElementById('curriculumResult');
    if(result)result.innerHTML=`<div class="package-summary"><h2>PAKET PERANGKAT AJAR LENGKAP</h2><p><b>${state.subject}</b> · ${state.level} Kelas ${state.grade}${state.section} · ${state.semester} · ${state.academicYear}</p><ol><li>CP lengkap + pemetaan elemen/kompetensi</li><li>TP terukur + indikator + asesmen</li><li>ATP berurutan + materi + alokasi</li><li>Program Semester berbasis minggu efektif</li><li>Program Tahunan berbasis distribusi satu tahun</li><li>Modul Ajar Pembelajaran Mendalam</li><li>LKPD + Bahan Ajar</li><li>Asesmen diagnostik, formatif, sumatif + kisi-kisi + kunci</li><li>Rubrik + KKTP</li><li>Diferensiasi + remedial + pengayaan</li><li>Refleksi siswa + guru + glosarium + daftar pustaka</li><li>Halaman pengesahan dan tanda tangan</li></ol><p class="muted">Modul Ajar sudah disimpan di browser. Buka menu Modul Ajar untuk mengedit, preview A4/F4, dan ekspor.</p><div class="module-actions"><button class="btn primary-dark" onclick="go('editor')">Buka & Edit Modul Ajar</button></div></div>`;
    showCurriculumSources([...new Set(allSources)],'Paket lengkap berhasil dibuat. Verifikasi CP/ATP dan sesuaikan kalender pendidikan serta kondisi satuan pendidikan sebelum digunakan.');
    if(mode)mode.textContent='Paket lengkap selesai';
    showToast('Paket perangkat ajar lengkap selesai dibuat');
  }catch(e){ if(mode)mode.textContent='Gagal'; showToast(e.message||'Gagal membuat paket lengkap'); }
}
